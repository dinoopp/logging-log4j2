...
algorithm.
+
The compression level can be specified using <<RolloverStrategy-attr-compressionLevel,`compressionLevel`>> configuration attribute.

`.gz`::
https://docs.oracle.com/javase/8/docs/api/java/util/zip/GZIPOutputStream.html[GZIP]
archive using the DEFLATE algorithm.
+
The compression level can be specified using <<RolloverStrategy-attr-compressionLevel,`compressionLevel`>> configuration attribute.

[#RolloverStrategy-compress-commons]
`.<algorithmName>`::
+
Log4j Core supports all the compression algorithms available through
https://commons.apache.org/proper/commons-compress/index.html[Apache Commons Compress].
To use these algorithms, an additional dependency is required:
+
--
...
[NOTE]
====
Apache Commons Compress currently supports the following values of `algorithmName`:

* `gzip`, `bzip2`, `deflate`, `snappy-framed`, `lz4-block`, `lz4-framed` are supported without **any** external dependency.
* `lzma`, `.xz` and `.zst` extensions require **additional** Commons Compress dependencies.
See
https://commons.apache.org/proper/commons-compress/index.html[Commons Compress documentation]
for more details.

The `pack200` algorithm is not a general purpose compression algorithm and has been disabled.
====
+
[TIP]
====
Support for additional compression algorithms can be obtained by extending Apache Commons Compress.

See
https://commons.apache.org/proper/commons-compress/examples.html#Extending_Commons_Compress_Compressors[Extending Commons Compress Compressors] for details.
====
...
| `false`
|
If `true`, no files will be deleted, but a
{log4j2-url}/manual/status-logger.html[status logger]
message of level `INFO` will be issued instead.
...
| The
{log4j2-url}/manual/status-logger.html[status logger]
to be used by diagnostic messages in the script.
...
<1> Adding
{log4j2-url}/manual/status-logger.html[status logger]
calls in your script might help in debugging it.
